            ...
            <div class="hidden md:flex flex-1 max-w-2xl mx-8" x-data="{ 
                ...
                <div class="relative w-full">
                    ...
                    <div x-show="showResults && searchQuery.length >= 2" 
                        ...
                        {{-- Error Message --}}
                        <div x-show="error" x-cloak class="px-4 py-6 text-center border-b border-gray-700 bg-red-900/20">
                            ...
                            <template x-for="book in searchResults" :key="book.id">
                                <a :href="`/products/${book.slug}`" 
                                    ...
                                    <div class="ml-3 flex-1 min-w-0">
                                        <p class="text-sm font-medium text-white truncate group-hover:text-emerald-400 transition-colors" x-text="book.name"></p>
                                        <p class="text-xs text-gray-400 truncate" x-text="book.author"></p>
                                        <div class="flex items-center justify-between mt-1">
                                            <p class="text-sm font-semibold text-emerald-500" x-text="'Rp ' + new Intl.NumberFormat('id-ID').format(book.price)"></p>
                                            <span class="text-xs text-gray-500" x-text="'Stok: ' + book.stock"></span>
                                        </div>
                                    </div>

                                    {{-- Arrow Icon --}}
                                    <svg class="w-5 h-5 text-gray-500 group-hover:text-emerald-400 transition-colors flex-shrink-0 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        {{-- No Results --}}
                        <div x-show="searchResults.length === 0 && !isSearching && !error" x-cloak class="px-4 py-8 text-center">
                            <svg class="w-12 h-12 mx-auto text-gray-600 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                            </svg>
                            <p class="text-gray-400 text-sm">Tidak ada hasil untuk "<span x-text="searchQuery" class="font-medium text-white"></span>"</p>
                            ...
                            <button @click="open = !open"
                                ...
                                <div class="relative">
                                    <img class="h-9 w-9 rounded-full object-cover border-2 border-gray-600 group-hover:border-emerald-500 transition-all duration-200"
                                        ...
                                        alt="{{ Auth::user()->name }}">
                                    <span class="absolute bottom-0 right-0 block h-2.5 w-2.5 rounded-full bg-emerald-500 border-2 border-gray-800"></span>
                                </div>
                                <span class="text-white text-sm font-medium hidden lg:block group-hover:text-emerald-400 transition-colors duration-200">
                                    {{ Auth::user()->name }}
                                </span>
