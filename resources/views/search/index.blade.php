...
@section('content')
    <style>
        /* Toggle Switch styling - Inline to ensure it loads */
        .toggle-switch {
            position: relative;
            display: inline-block;
            width: 44px;
            ...
            @if ($products->count() > 0)
                        ...
                        <div class="p-6">
                            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                                ...
                                <div class="lg:col-span-3">
                                    ...
                                    <div>
                                        <label class="flex items-center justify-between cursor-pointer group bg-gray-700/30 rounded-xl px-4 py-3 hover:bg-gray-700/50 transition-all">
                                            <span class="text-base text-gray-300 group-hover:text-white transition-colors">Hanya yang tersedia</span>
                                ...
                                <div class="hidden lg:block lg:col-span-1">
                                    <div class="h-full w-px bg-gradient-to-b from-transparent via-gray-700/50 to-transparent"></div>
                                </div>
                                    ...
                                    <div class="w-full space-y-2">
                                        ...
                                        <div class="flex flex-wrap items-center gap-2 min-h-[28px]">
                                            ...
                                            @if(request('stock') == 'all')
                                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-gray-500/10 text-gray-300 text-xs font-medium rounded-lg border border-gray-500/20">
                                                    Semua Produk
                                                </span>
                                            @elseif(request('stock') || request('sort'))
                                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-emerald-500/10 text-emerald-300 text-xs font-medium rounded-lg border border-emerald-500/20">
                                                    Stok Tersedia
                                                </span>
                                            @endif

                                            @if(request('sort'))
                                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-cyan-500/10 text-cyan-300 text-xs font-medium rounded-lg border border-cyan-500/20">
                                                    @php
                                                        $sortLabels = [
                                                            'newest' => 'Terbaru',
                                                            'oldest' => 'Terlama',
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    @foreach ($products as $product)
                        <div class="relative bg-gray-800/40 backdrop-blur-sm rounded-2xl shadow-xl overflow-hidden transform hover:scale-[1.02] transition-all duration-300 hover:shadow-2xl hover:shadow-emerald-500/20 border border-gray-700/30 group">
                            ...
                            <a href="{{ url('/products/' . $product->slug) }}" class="block">
                                ...
                                <div class="absolute top-3 left-3 right-3 flex items-start justify-between z-10">
                                    ...
                                    {{-- Stock Badge --}}
                                    @if ($product->stock > 0)
                                        <span class="inline-flex items-center px-2 py-1 bg-emerald-500/90 backdrop-blur-sm text-white text-xs font-semibold rounded-lg shadow-lg">
                                            <span class="w-1.5 h-1.5 bg-white rounded-full mr-1.5 animate-pulse"></span>
                                            Tersedia
                                        </span>
                                    @else
                                        <span class="inline-flex items-center px-2 py-1 bg-gray-600/90 backdrop-blur-sm text-gray-300 text-xs font-semibold rounded-lg shadow-lg">
                                ...
                                <div class="relative overflow-hidden bg-gray-900/20">
                                    <img src="{{ $product->image ? asset('storage/' . $product->image) : 'https://via.placeholder.com/640x480?text=No+Image' }}"
                                        alt="{{ $product->name }}"
                                ...
                                <div class="p-5">
                                    {{-- Title --}}
                                    <h3 class="text-lg font-bold text-white mb-2 line-clamp-2 group-hover:text-emerald-400 transition-colors duration-300 min-h-[3.5rem]">
                                        {{ $product->name }}
                                    </h3>
                                    ...
                                    <div class="flex items-center justify-between mb-4">
                                        ...
                                        @if ($product->stock > 0)
                                            <p class="text-xs text-gray-400">
                                                <span class="font-semibold text-emerald-400">{{ $product->stock }}</span> stok
                                            </p>
                                        @endif
                                    </div>
                                </div>
            ...
            @else
                ...
                <div class="flex flex-col items-center justify-center py-20">
                    <div class="relative mb-8">
                        <div class="w-32 h-32 bg-gray-800/50 rounded-full flex items-center justify-center border-4 border-gray-700/50">
                            <svg class="w-16 h-16 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                    d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                            </svg>
                        </div>
                        <div class="absolute -bottom-2 -right-2 w-12 h-12 bg-red-500/20 rounded-full flex items-center justify-center border-2 border-red-500/50">
